...
def _assert_isinstance(ctx, v, *types):
    if not isinstance(v, types):
        raise error.SerializationError(ctx, f'{v} <{type(v).__qualname__}> is not an instance of {" or ".join(T.__qualname__ for T in types)}')
...
class Custom(typing.Generic[T]):
    ...
    def loads(self, s: str, ctx: str = '') -> T:
        try:
            return self.C.__stringly_loads__(s)
        except Exception as e:
            raise error.SerializationError(ctx, f'failed to deserialize {s!r}: {e}') from None

    def dumps(self, v: T, ctx: str = '') -> str:
...
class Boolean:
    def loads(self, s: str, ctx: str = '') -> bool:
        v = dict(true=True, yes=True, false=False, no=False).get(s.lower())
        if v is None:
            raise error.SerializationError(ctx, f'invalid boolean value {s!r}')
        return v

    def dumps(self, v: bool, ctx: str = '') -> str:
        _assert_isinstance(ctx, v, bool)
...
class Native:
    ...
    def loads(self, s: str, ctx: str = '') -> int:
        try:
            v = self.T(s)
        except Exception as e:
            raise error.SerializationError(ctx, e) from None
        return v

    def dumps(self, v, ctx: str = ''):
        _assert_isinstance(ctx, v, self.T, *self.alt)
        s = str(self.T(v))
...
class UniformTuple(typing.Generic[T]):
    ...
    def loads(self, s: str, ctx: str = '') -> typing.Tuple[T,...]:
        parts = util.safesplit(s, ',')
        return tuple(self.itemserializer.loads(util.unprotect(si), f'{ctx}:{i+1}') for i, si in enumerate(parts))

    def dumps(self, v: typing.Tuple[T, ...], ctx: str = '') -> str:
        return ','.join(util.protect_regex(self.itemserializer.dumps(vi, f'{ctx}:{i+1}'), ',') or '{}' for i, vi in enumerate(v))
...
class PluriformTuple:
    ...
    def loads(self, s: str, ctx: str = '') -> typing.Tuple[typing.Any, ...]:
        parts = util.safesplit(s, ',')
        if len(self.itemserializers) == len(parts):
            return tuple(zi.loads(util.unprotect(si), f'{ctx}:{i+1}') for i, zi, si in zip(range(len(parts)), self.itemserializers, parts))
        raise error.SerializationError(ctx, 'tuple has incorrect length')

    def dumps(self, v: typing.Tuple[typing.Any, ...], ctx: str = '') -> str:
        if len(self.itemserializers) == len(v):
            return ','.join(util.protect_regex(zi.dumps(vi, f'{ctx}:{i+1}'), ',') or '{}' for i, zi, vi in zip(range(len(v)), self.itemserializers, v))
        raise error.SerializationError(ctx, 'tuple has incorrect length')
...
class Dict(typing.Generic[K, V]):
    ...
    def loads(self, s: str, ctx: str = '') -> typing.Dict[K, V]:
        v: typing.Dict[K, V] = {}
        for si in util.safesplit(s, ','):
            parts = util.safesplit(si, '=', 1)
            if len(parts) != 2:
                raise error.SerializationError(ctx, 'missing value')
            key, value = map(util.unprotect, parts)
            v[self.keyserializer.loads(key, f'{ctx}:?')] = self.valueserializer.loads(value, f'{ctx}:{key}')
        return v

    def dumps(self, v: typing.Dict[K, V], ctx: str = '') -> str:
        return ','.join(util.protect_regex(self.keyserializer.dumps(vk, f'{ctx}:?'), ',|=') + '=' + util.protect_regex(self.valueserializer.dumps(vv, f'{ctx}:{vk}'), ',') for vk, vv in v.items())
...
class Union:
    ...
    def loads(self, s: str, ctx: str = '') -> typing.Any:
        name, value = util.splitarg(s)
        if name not in self.serializers:
            raise error.SerializationError(ctx, f'unknown type: {name}')
        return self.serializers[name].loads(value, f'{ctx}({name})')

    def dumps(self, v: typing.Any, ctx: str = '') -> str:
        for name, serializer in self.serializers.items():
            try:
                s = serializer.dumps(v, ctx)
            except error.SerializationError:
                continue
            return name + util.protect_unconditionally(s) if s else name
        raise error.SerializationError(ctx, 'failed to find matching serializer')
...
class Optional(typing.Generic[T]):
    ...
    def loads(self, s: str, ctx: str = '') -> typing.Optional[T]:
        if s == '':
            return None
        return self.serializer.loads(util.unprotect(s), ctx)

    def dumps(self, v: typing.Optional[T], ctx: str = '') -> str:
        if v is None:
            return ''
        s = self.serializer.dumps(v, ctx)
...
class Sequence:
    ...
    def loads(self, s: str, ctx: str = '') -> typing.Any:
        return self.origin(self.itemserializer.loads(util.unprotect(si), f'{ctx}[{i}]') for i, si in enumerate(util.safesplit(s, ',')))

    def dumps(self, v: typing.Any, ctx: str = '') -> str:
        return ','.join(util.protect_regex(self.itemserializer.dumps(vi, f'{ctx}[{i}]'), ',') or '{}' for i, vi in enumerate(v))
...
class Enum(typing.Generic[enumT]):
    ...
    def loads(self, s: str, ctx: str = '') -> enumT:
        try:
            return self.cls.__members__[s]
        except KeyError:
            raise error.SerializationError(ctx, f'invalid enum value {s!r}') from None

    def dumps(self, v: enumT, ctx: str = '') -> str:
        _assert_isinstance(ctx, v, self.cls)
        return v.name
...
class Generic(typing.Generic[T]):
        ...
        if not s:
            ...
            if not self.npositional:
                ...
                if len(parts) != 2 or parts[0] != self.argnames[0]:
                    raise error.SerializationError(ctx, f'invalid argument for {self.cls.__name__}: {s}') from None
                s = parts[1]
            ...
            for si in util.safesplit(s, ','):
                ...
                if len(parts) == 2:
                    ...
                    try:
                        ...
                        raise error.SerializationError(ctx, f'invalid argument {name!r}') from None
                    args[index] = _strarg(value)
                elif index < self.npositional:
                    args[index] = _strarg(util.unprotect(si))
                    index += 1
                else:
                    raise error.SerializationError(ctx, 'invalid expression')
        for i, arg in enumerate(args):
            if arg is inspect.Parameter.empty:
                raise error.SerializationError(ctx, f'missing mantatory argument {self.argnames[i]!r}')
            if isinstance(arg, _strarg):
                args[i] = self.serializers[i].loads(arg.value, f'{ctx}.{self.argnames[i]}')
    ...
    def dumps(self, v: T, ctx: str = '') -> str:
        ...
        if hasattr(self.cls, '__getnewargs_ex__'):
            ...
            raise error.SerializationError(ctx, f'cannot dump {v}')
        dumps = [serializer.dumps(arg, f'{ctx}.{argname}') for serializer, arg, argname in zip(self.serializers, args, self.argnames)]
